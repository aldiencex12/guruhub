...
export class ImportController {
  ...
  private buildExcelResponse(headers: string[], filename: string, set: any) {
  ...
  templateStudents = async ({ set }: any) => {
    return this.buildExcelResponse(["nisn", "name", "gender", "religion", "className"], "template-students.xlsx", set);
  };

  templateTeachers = async ({ set }: any) => {
    return this.buildExcelResponse(["nip", "name", "gender", "phone"], "template-teachers.xlsx", set);
  };

  templateClasses = async ({ set }: any) => {
    return this.buildExcelResponse(["name", "gradeLevel", "homeroomTeacherNip"], "template-classes.xlsx", set);
  };

  templateSubjects = async ({ set }: any) => {
    return this.buildExcelResponse(["code", "name", "gradeLevel"], "template-subjects.xlsx", set);
  };

  templateClassMembers = async ({ set }: any) => {
    return this.buildExcelResponse(["className", "nisn"], "template-class-members.xlsx", set);
  };


  templateSchedules = async ({ set }: any) => {
    return this.buildExcelResponse(["day", "startTime", "endTime", "className", "subjectCode", "teacherNip"], "template-schedules.xlsx", set);
  };
}
