...
export class ImportService {
  ...
  async importStudents(file: File, schoolId: number, userId: number): Promise<any> {
    ...
    await this.repo.runTransaction(async (tx) => {
      ...
      if (ay) {
        ...
        const memberSet = new Set(existingMemberships.map(m => `${m.studentId}-${m.classId}`));
  ...
  private async validateStudents(rows: any[], schoolId: number): Promise<ImportError[]> {
    ...
    const dbNisn = new Set<string>(dbNisnsGlobal.map((s) => s.nisn));
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      if (!r.nisn) {
        ...
        if (r.nisn.length < 10) {
          errors.push({ row: rowNum, column: "nisn", reason: "NISN minimal 10 digit" });
        } else if (r.nisn.length > 20) {
